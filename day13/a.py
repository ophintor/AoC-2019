...
def arcade_game(program):
    ...
    while not finished:
        x,position,offset = intcode(program,0,position,offset)

        if x == -1:
            finished = 1
        else:
            y,position,offset = intcode(program,0,position,offset)
            tile_id,position,offset = intcode(program,0,position,offset)
            if tile_id == 2: count += 1
